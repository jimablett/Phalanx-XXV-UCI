...
#define update_hash(p, s)                                                      \
  {                                                                            \
    G[Counter].hashboard ^= H[HP[p]][HS[s]];                                   \
  }
...
void do_move(register tmove *m) {
  ...
  case 0: /* most of the moves */
    ...
    {
      static unsigned char cb_[80] = {0,      WLONG,
                                      0,      0,
                                      0,      (WSHORT | WLONG),
                                      0,      0,
                                      WSHORT, 0,
  ...
  case SHORT_CASTLING:
    if (Color == WHITE) {
      ...
      update_hash(WR, F1);
      L[G1] = L[E1];
      WKP = L[L[G1].next].prev = G1;
      L[F1] = L[H1];
      L[L[F1].prev].next = L[L[F1].next].prev = F1;
    } else {
      B[E8] = 0;
      B[F8] = BR;
      B[G8] = BK;
      ...
      update_hash(BR, F8);
      L[G8] = L[E8];
      BKP = L[L[G8].next].prev = G8;
      L[F8] = L[H8];
      L[L[F8].prev].next = L[L[F8].next].prev = F8;
    }
    break;
  case LONG_CASTLING:
    if (Color == WHITE) {
      ...
      update_hash(WR, D1);
      L[C1] = L[E1];
      WKP = L[L[C1].next].prev = C1;
      L[D1] = L[A1];
      L[L[D1].prev].next = L[L[D1].next].prev = D1;
    } else {
      B[A8] = 0;
      B[C8] = BK;
      B[D8] = BR;
      ...
      update_hash(BR, D8);
      L[C8] = L[E8];
      BKP = L[L[C8].next].prev = C8;
      L[D8] = L[A8];
      L[L[D8].prev].next = L[L[D8].next].prev = D8;
    }
    break;
...
void undo_move(register tmove *m) {
  ...
  case SHORT_CASTLING:
    if (Color == WHITE) {
      ...
      B[H1] = WR;
      L[E1] = L[G1];
      WKP = L[L[E1].next].prev = E1;
      L[H1] = L[F1];
      L[L[H1].prev].next = L[L[H1].next].prev = H1;
    } else {
      B[E8] = BK;
      B[F8] = 0;
      B[G8] = 0;
      B[H8] = BR;
      L[E8] = L[G8];
      BKP = L[L[E8].next].prev = E8;
      L[H8] = L[F8];
      L[L[H8].prev].next = L[L[H8].next].prev = H8;
    }
    break;
  case LONG_CASTLING:
    if (Color == WHITE) {
      ...
      WKP = L[L[E1].next].prev = E1;
      L[A1] = L[D1];
      L[L[A1].prev].next = L[L[A1].next].prev = A1;
    } else {
      B[A8] = BR;
      B[C8] = 0;
      B[D8] = 0;
      ...
      BKP = L[L[E8].next].prev = E8;
      L[A8] = L[D8];
      L[L[A8].prev].next = L[L[A8].next].prev = A8;
    }
    break;
  default: /* en passant pawn capture */
    B[m->to] = 0;
